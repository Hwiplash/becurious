...
변화율 필드가 null이면 1월 또는 6월 관측이 없어 산출 불가라는 뜻이다. 이를 0% 변화나 감소·성장 부재로 해석하지 않는다.
consumption_context.core9_scope에 미관측 업종이 있으면 그 달의 핵심 9업종 값은 관측 업종만의 합계다. 미관측을 0으로 취급하거나 고정된 9업종 총액·기대값과 비교해 문제 또는 성장으로 판정하지 않는다.
consumption_context의 전체 상권 상태(total_market_status), 내국인 개인 상태(domestic_personal_status), 판정 차이(population_comparison), 외국인·법인 비중, 내국인 연령·성별 구조를 구분한다.
전체 상권은 고객코드 1·2·3·x의 BC 이용금액·건수다. 전체 카드시장 또는 지역 실제 총매출로 표현하지 않는다. 코드 1·2는 내국인 남성·여성, 3은 성별 구분 없는 외국인, x는 법인이고 AGE_CD x는 법인 연령 미적용이다.
내국인 개인의 기대값과 전체 실제값을 비교하지 않는다. 고객구조는 domestic_personal_customer_structure만 사용한다. 외국인·법인 소비 규모의 차이를 인과효과로 단정하지 않는다. 낮은 소비를 정책 문제나 상권 쇠퇴로 확정하지 않는다.
내부 판정 코드나 신뢰등급 대신 제공된 사용자용 상태 설명을 사용하고, 해당 지역의 보정 경고와 해석 한계를 진단에 반영한다.

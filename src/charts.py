...
def style(fig: go.Figure, height: int = 340) -> go.Figure:
    fig.update_layout(
        ...
        font=dict(color="#C8D5E3"), legend_title_text="",
        hoverlabel=dict(bgcolor="#12243A", font_color="white"),
    )
...
def monthly_trend(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(yaxis=dict(title="매출액(원)", tickformat="~s"), yaxis2=dict(title="건수", overlaying="y", side="right", tickformat="~s"), hovermode="x unified")
...
def industry_bar(df: pd.DataFrame, n: int = 10) -> go.Figure:
    ...
    fig.update_layout(coloraxis_showscale=False, xaxis_title="매출액(원)", yaxis_title=None)
...
def segment_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title=None, yaxis_title="매출액(원)")
